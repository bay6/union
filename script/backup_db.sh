...
30 4 * * * env RBENV_VERSION=jruby-1.6.7.2 bash -l -c 'cd /Users/kenlu/git_repo/union && mysqldump --user=root --password="" --databases union_development > db/`date +\%Y-\%m-\%d`_backup_database.sql'